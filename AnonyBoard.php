			...
			<a href="AnonyBoard.php"><tt>Board</tt></a>
				<table class="one" width="70%">
                    ...
                    <?php
                        ...
                        $query="select count(*) from Anonyboard";
                        ...
                        $query="select * from anonyboard order by bnum desc limit $offset, $list_num";
                        $result=mysql_query($query);
					...
					<tr>
						...
						<td align="right">
							<input type='button' id="button" value='글쓰기' onclick="location.href='AnonyBoard_write.php'">
						</td>
					</tr>
				</table>
				<table>
...
if($block > 1) {
    $prev=$first-1;
    echo "<a href='AnonyBoard.php?page=1'>[처음]</a>&nbsp; ";
}

//[이전]
if($page > 1) {
    $go_page=$page-1;
    echo " <a href='AnonyBoard.php?page=$go_page'><</a>&nbsp; ";
}
...
for ($page_link=$first+1;$page_link<=$last;$page_link++) {
    if($page_link==$page) {
        ...
        echo "&nbsp;<a href='AnonyBoard.php?page=$page_link'>$page_link</a>&nbsp;";
    }
}
...
if($block < $total_block) {
    ...
    $next=$last+1;
    echo "<a href='AnonyBoard.php?page=$total_page'>[마지막]</a></p>";

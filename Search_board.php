...
<html>
	<head>
    	...
    	<style>
            ...
            table.one thead th {
                ...
                vertical-align: top;
                color: #369;
                border-bottom: 5px solid #73c1e3;
            }
            table.one tbody th {
				font-size: 13px;
                font-weight: bold;
	...
	<body bgcolor="#73c1e3">
		...
		<center>
			<a href="Board.php"><tt>Board</tt></a>
				<table class="one" width="70%">
                    ...
                    <?php
                        ...
                        if($s_kind == 'nick'){
                            $query="select count(*) from board where nick like '%$s_text%' order by bnum desc limit $offset, $list_num";
                        }
                        else{
                            $query="select count(*) from board where title like '%$s_text%' order by bnum desc limit $offset, $list_num";
                        }
                        ...
                        if($s_kind == 'nick'){
                            $query="select * from board where nick like '%$s_text%' order by bnum desc limit $offset, $list_num";
                        }
                        else{
                            $query="select * from board where title like '%$s_text%' order by bnum desc limit $offset, $list_num";
                        }
                        $result=mysql_query($query);
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <p align=center>$array[nick]</p>
                            </td>
                            <td>
                                <p align=center>$array[date]</p>
                            </td>
                        </tr>
                        <form method='post' name='form$cur_num' action='Board_content.php'>
				...
				<table>
					<tr>
					<form action='Search_board.php' method='post'>
    					<td>
...
if($block > 1) {
    $prev=$first-1;
    echo "<a href='Search_Board.php?page=1'>[처음]</a>&nbsp; ";
}

//[이전]
if($page > 1) {
    $go_page=$page-1;
    echo " <a href='Search_Board.php?page=$go_page'><</a>&nbsp; ";
}
...
for ($page_link=$first+1;$page_link<=$last;$page_link++) {
    if($page_link==$page) {
        ...
        echo "&nbsp;<a href='Search_Board.php?page=$page_link'>$page_link</a>&nbsp;";
    }
}
...
if($block < $total_block) {
    ...
    $next=$last+1;
    echo "<a href='Search_Board.php?page=$total_page'>[마지막]</a></p>";
